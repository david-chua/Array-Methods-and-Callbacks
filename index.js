...
function getYears(cb) {
    let years = cb.map(year => year.Year);
    return years;
}
...
function getAverageGoals(getFinals) {
   /* code here */
   const grandTotal = getFinals.reduce(
		(total, item) => total + item["Home Team Goals"] + item["Away Team Goals"],
		0,
	);
	return (grandTotal / getFinals.length).toFixed(2);
